...
def convert_depth_to_pointcloud(observations):
    ...
    # f_ang = ((x-cx)**2 + (y-cy)**2 + fx**2)**0.5

    # x3 = (x - cx) * depth_image * 1 / f_ang
    # y3 = (y - cy) * depth_image * 1 / f_ang
    # z3 = fx * depth_image / f_ang
    ...
    # print(y3.min(), y3.max())
    # print(((y - cy) / fy).max(), ((y - cy) / fy).min())
    # print(y.max(), y.min(), fy, cy)

    # pack the point cloud according to the votenet camera coordinate system
    # which is actually the same as benchbot's

    point3d = np.stack((x3.flatten(), z3.flatten(), -y3.flatten()),
                       axis=1)

    return point3d


def votenet_detection(net, observations, T1, T2):
    ...
    if detections[0]:
        ...
        for detection in detections:
            ...
            box_corners_camera = np.concatenate(
                ...
                axis=1)
            # print(666, box_corners_camera)
            box_corners_camera[:, 1] *= -1
            
            box_corners_world = T1 @ T2 @ box_corners_camera[:, [0, 1, 2, 3]].T
            box_corners_world = box_corners_world.T
