...
class ActiveNeuralSLAMExplorer(ActiveNeuralSLAMBase):
    ...
    def act(
        ...
        visited_states = self._update_state_visitation(prev_state_estimates["visited_states"], map_xy)  # (bs, 1, M, M)
        ...
        if ep_step > 0:
            ...
            curr_dist2localgoal = self._compute_dist2localgoal(global_map, map_xy, self.states["curr_local_goals"])

            prev_map_position = self.states["curr_map_position"]
            prev_step_size = (torch.norm(curr_map_position - prev_map_position, dim=1).unsqueeze(1) * s)
